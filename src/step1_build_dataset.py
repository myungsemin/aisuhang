...
def map_rtype(s):
    """발생유형(해경·KMST 표기 혼용) → 위험유형. 문자열 포함으로 강건하게 매핑."""
    s = str(s)
    if "충돌" in s: return "collision"
    if "좌초" in s or "좌주" in s or "접촉" in s: return "grounding"
    if any(k in s for k in ["기관", "추진", "조타", "부유물", "표류", "운항저해", "키손상"]): return "engine"
    if "침수" in s or "침몰" in s or "전복" in s: return "capsize"
    return None
...
def load_accidents():
    ...
    # 중앙해양안전심판원 해양사고 통계현황(2021~2025, 5년) — 사고 표본 대폭 보강
    fkm = glob.glob(os.path.join(DATA_DIR, "**", "*통계현황_2025*.csv"), recursive=True)
    if fkm:
        d = read_csv_any(fkm[0]); d.columns = [c.strip().replace(" ", "") for c in d.columns]
        def dms2(deg, m, s):
            try:
                return float(re.sub("[NSEW]", "", str(deg))) + float(m)/60 + float(s)/3600
            except Exception:
                return np.nan
        d["lat"] = [dms2(a, b, c) for a, b, c in zip(d["해양사고장소(위도)"], d["해양사고장소(위분)"], d["해양사고장소(위초)"])]
        d["lon"] = [dms2(a, b, c) for a, b, c in zip(d["해양사고장소(경도)"], d["해양사고장소(경분)"], d["해양사고장소(경초)"])]
        yr = pd.to_numeric(d["해양사고발생(년도)"], errors="coerce").fillna(2021).clip(2000, 2030).astype(int)
        mo = pd.to_numeric(d["해양사고발생(월)"], errors="coerce").fillna(1).clip(1, 12).astype(int)
        hh = pd.to_numeric(d["해양사고발생(시)"], errors="coerce").fillna(0).clip(0, 23).astype(int)
        d["dt"] = pd.to_datetime(dict(year=yr, month=mo, day=1, hour=hh), errors="coerce")
        d["발생유형"] = d["해양사고종류1"]
        d["rtype"] = d["해양사고종류1"].map(map_rtype)
        d["bad_wx"] = False
        d["src"] = "kmst"
        rows.append(d[["lat", "lon", "dt", "발생유형", "rtype", "bad_wx", "src"]])
...
def build():
    print("· 사고 데이터 로드 중...")
    acc = load_accidents()
    # 누수 방지 연도 분할: 라벨=최근 3년(2023~2025), 과거이력(KDE)=2022년 이하
    acc25 = acc[acc.year >= 2023]      # 라벨용(최근, 표본 풍부)
    acc10 = acc[acc.year <= 2022]      # 과거 이력·사고밀도용(누수 없음)
    print(f"  - 연구해역 사고: 라벨(2023~2025) {len(acc25)}건 / 과거(≤2022) {len(acc10)}건")
